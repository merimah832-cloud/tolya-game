...
export const TolyaImage = ({ isJumping, direction }: { isJumping: boolean, direction: 'left' | 'right' }) => {
    ...
    return (
      <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
        <g transform={direction === 'left' ? 'scale(-1, 1) translate(-100, 0)' : ''}>
          <circle cx="50" cy="20" r="15" fill="#ffdbac" />
          ...
          <rect x="40" y="35" width="20" height="35" rx="5" fill="#3b82f6" />
          <path d={isJumping ? "M40 40 L20 30" : "M40 40 L30 60"} stroke="#ffdbac" strokeWidth="6" strokeLinecap="round" />
          <path d={isJumping ? "M60 40 L80 20" : "M60 40 L70 60"} stroke="#ffdbac" strokeWidth="6" strokeLinecap="round" />
...
export const BedImage = ({ variant = 0, isGiant = false }: { variant?: number, isGiant?: boolean }) => {
  const [error, setError] = useState(false);
  ...
  const imageSrc = isGiant ? '/assets/boss.png' : images[variant % images.length];
